function demo_moeaddda_withPSarchive(problem,times)
% demo('tec09_f1')
...
init_moeaddda_withPSarchive('problem', mop, 'popsize', popsize, 'niche', 20, 'pns', 0.9, 'F', 0.5, 'method', 'ts', 'updatesize', 2);
% updateplot(0);
for g=1:1000
    step_moeaddda_withPSarchive(mop, g);
%     updateplot(g);
    if mod(g,10)==0
        sname = sprintf('data/moeaddda_withPSarchive/%s_run%d_gen%d', problem, times, g);
        savearchival(sname);
    end
end
endt    = toc;   
